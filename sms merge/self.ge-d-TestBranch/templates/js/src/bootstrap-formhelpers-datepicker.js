...
+function ($) {
  ...
  BFHDatePicker.prototype = {
    ...
    checkWeekDay: function (day, month, year) {
      var WeekDays,
              WeekDay;
      WeekDays = $.trim(this.options['weekday'].toString());
      ...
      WeekDay = getDayOfWeek(month, year, day).toString();
    ...
    updateCalendarDays: function ($calendar, month, year) {
      ...
      for (day = 1; day <= numDaysCurrentMonth; day = day + 1) {
        ...
        {
          row += '<td data-day="' + day + '" class="off">' + day + '</td>';
        } else if (this.checkWeekDay(day, month, year))
        {
          row += '<td data-day="' + day + '" class="off">' + day + '</td>';
