...
class XCurl
{
	...
	public function buildPostData( $data )
	{
		...
		{
			$data = XCurlEncoder::encodeJson( $data );
		}
		elseif ( is_array( $data ) )
		{
			...
			// referenced.
			if ( XCurlArrayUtil::is_array_multidim( $data ) )
			{
				$data = XCurlArrayUtil::array_flatten_multidim( $data );
			}
	...
	public function exec( $ch = null )
	{
		...
		{
			$this->curlErrorMessage = curl_strerror( $this->curlErrorCode ) . (
			...
			if ( isset( $this->options[CURLOPT_CUSTOMREQUEST] ) )
			{
				if ( (version_compare( PHP_VERSION, '5.5.11' ) < 0) || defined( 'HHVM_VERSION' ) )
				{
					trigger_error(
									...
									. 'php-curl-class Curl object or upgrade your PHP engine.', E_USER_ERROR
					);
				}
				else
				{
	...
	{
		$this->jsonDecoder = 'XCurlDecoder::decodeJson';
	...
	{
		$this->xmlDecoder = 'XCurlDecoder::decodeXml';
	...
	public function setDefaultDecoder( $mixed = 'json' )
	{
		...
		else
		{
			if ( $mixed === 'json' )
			{
				$this->defaultDecoder = 'XCurlDecoder::decodeJson';
			}
			elseif ( $mixed === 'xml' )
			{
				$this->defaultDecoder = 'XCurlDecoder::decodeXml';
			}
		}
	...
	public function setUrl( $url, $mixed_data = '' )
	{
		...
		if ( $this->url === null )
		{
			$this->url = (string) new XCurlUrl( $built_url );
		}
		else
		{
			$this->url = (string) new XCurlUrl( $this->url, $built_url );
		}
	...
	private function parseHeaders( $raw_headers )
	{
		...
		$http_headers = new XCurlCaseInsensitiveArray();
	 ...
	 * @param  $raw_headers
	 *
	 * @return \Curl\XCurlCaseInsensitiveArray
	 */
	private function parseRequestHeaders( $raw_headers )
	{
		$request_headers = new XCurlCaseInsensitiveArray();
	...
	private function parseResponseHeaders( $raw_response_headers )
	{
		...
		$response_headers = new XCurlCaseInsensitiveArray();
	...
	private function initialize( $base_url = null )
	{
		...
		$this->setOpt( CURLOPT_RETURNTRANSFER, true );
		$this->headers = new XCurlCaseInsensitiveArray();
