...
class LogoutRequest
{
	 ...
	 */
	public function __construct( XSAMLSettings $settings, $request = null, $nameId = null, $sessionIndex = null, $nameIdFormat = null, $nameIdNameQualifier = null, $nameIdSPNameQualifier = null )
	{
		...
		if ( !isset( $request ) || empty( $request ) )
		{
			...
			if ( !empty( $nameId ) )
			{
				if ( empty( $nameIdFormat ) && $spData['NameIDFormat'] != Constants::NAMEID_UNSPECIFIED )
				{
	...
	public static function getID( $request )
	{
		...
		if ( false === $dom )
		{
			throw new XSAMLError(
											...
											XSAMLError::SAML_LOGOUTREQUEST_INVALID
			);
		}
		...
		if ( $encryptedEntries->length == 1 )
		{
			...
			if ( empty( $key ) )
			{
				throw new XSAMLError(
												...
												XSAMLError::PRIVATE_KEY_NOT_FOUND
				);
			}
	...
	public function isValid( $retrieveParametersFromServer = false )
	{
		...
		try
		{
			...
			if ( $this->_settings->isStrict() )
			{
				...
				$Signature = Request::getVar( 'Signature', null, 'get' );
				if ( $security['wantMessagesSigned'] && !$Signature )
				{
					throw new ValidationError(
			...
			{
				$GET = Request::get( 'get' );
				$signatureValid = Utils::validateBinarySign( "SAMLRequest", $GET, $idpData, $retrieveParametersFromServer );
				if ( !$signatureValid )
				{
					throw new ValidationError(
