...
class Utils
{
	...
	public static function loadXML( DOMDocument $dom, $xml )
	{
		...
		foreach ( $dom->childNodes as $child )
		{
			...
			{
				throw new Exception( 'Detected use of DOCTYPE/ENTITY in XML, disabled to prevent XXE/XEE attacks' );
			}
		}
	...
	public static function redirect( $url, array $parameters = array(), $stay = false )
	{
		...
		if ( $wrongProtocol || empty( $url ) )
		{
			throw new XSAMLError(
											...
											XSAMLError::REDIRECT_INVALID_URL
			);
		}
	...
	{
		$URI = URI::getInstance();
		return $URI->toString( array( 'scheme', 'user', 'pass', 'host', 'port' ) );
	...
	{
		return 'SAML_' . sha1( random_bytes( 20 ) );
	...
	public static function calculateX509Fingerprint( $x509cert, $alg = 'sha1' )
	{
		...
		switch ( $alg )
		{
			...
			case 'sha1':
			default:
				$fingerprint = mb_strtolower( sha1( $decodedData ) );
				break;
		}
		return $fingerprint;
	...
	public static function formatFingerPrint( $fingerprint )
	{
		...
		$formatedFingerprint = mb_strtolower( $formatedFingerprint );
