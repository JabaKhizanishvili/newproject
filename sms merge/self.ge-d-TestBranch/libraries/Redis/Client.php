...
class Credis_Client
{
	...
	public function connect()
	{
		...
		if ( $this->standalone )
		{
			...
			$remote_socket = $this->port === NULL ? $this->scheme . '://' . $this->host : $this->scheme . '://' . $this->host . ':' . $this->port;
		...
		else
		{
			...
			try
			{
				if ( $this->oldPhpRedis )
				{
					$result = $this->persistent ? $this->redis->pconnect( $this->host, (int) $this->port, $socketTimeout, $this->persistent ) : $this->redis->connect( $this->host, (int) $this->port, $socketTimeout );
				}
				else
				{
					...
					$result = $this->persistent ? $this->redis->pconnect( $this->scheme . '://' . $this->host, (int) $this->port, $socketTimeout, $this->persistent, 0, 0.0, $context ) : $this->redis->connect( $this->scheme . '://' . $this->host, (int) $this->port, $socketTimeout, null, 0, 0.0, $context );
				}
			}
			catch ( Exception $e )
			{
