...
class ExcelXml
{
	...
	public function __construct( $file, $col_count, $row_count )
	{
		...
		$this->_filename = File::makeSafe( $filename );

		$folder = dirname( $file );
		$foldername = str_replace( '/', DS, $folder );
		$this->_foldername = $foldername;

		if ( !is_dir( $this->_foldername ) )
		{
			mkdir( $this->_foldername, 0777 );
		}
...
//            $type = preg_match('/^([\d]+)$/', $str) ? 'Number' : 'String';
		...
		{
			$write = file_put_contents( $this->_foldername . DS . $this->_filename, $str, FILE_APPEND );
			return $write;
		...
		{
			$content = file_get_contents( $this->_foldername . DS . $this->_filename );
...
//			header( "Content-Length: " . strlen( $content ) );
		...
		{
			return str_replace( DS, '/', $this->_foldername ) . '/' . $this->_filename;
