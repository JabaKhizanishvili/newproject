...
require_once 'elements' . DS . 'filter.php';
...
class Filters extends Registry
{
	/**
	...
	public function __construct( $data, $path = '', $config = array() )
	{
		...
		// Set base path
		$this->_elementPath[] = dirname( __FILE__ ) . DS . 'elements' . DS . 'filter';
		if ( is_array( $data ) )
		{
			$this->_data = $data;
		}
	...
	public function render( $group = '_default' )
	{
		...
		$type = $this->_xml[$group]->attributes( 'type' );
		ob_start();
		if ( $type == 'h' )
		{
//			$this->StartTable( $group );
			$this->getHorizontalTable( $group );
//			$this->EndTable( $group );
		}
		else
		{
			$this->getVerticalTable( $group );
		}
//		$this->EndTable( $group );
	 ...
	 * @since	1.5
	 */
	public function getHorizontalTable( $group = '_default' )
	{
		if ( !isset( $this->_xml[$group] ) )
		{
			return false;
		...
		<div class="row filter-block">
			...
			foreach ( $children as $param )
			{
				...
				$must = trim( $param->attributes( 'must' ) );
				$type = trim( $param->attributes( 'type' ) );
				$id = $type . '_' . $name;
				$permission = trim( $param->attributes( 'permission' ) );

				if ( !empty( $permission ) && !Helper::CheckTaskPermision( $permission ) )
				{
					continue;
				}
				?>
				<div class="col-md-4 col-lg-3 col-sm-6 col-xs-12 <?php echo $class; ?>" style="min-height: 76px;" >
					<div  class="page_filter_caption">
						<label for="<?php echo $id; ?>">
							...
							{
								?>
								<span class="label-must f_must_dot" >
	 ...
	 * @since	1.5
	 */
	public function getParam( $node, $group = '_default' )
	{
		//get the type of the parameter
		$name = trim( $node->attributes( 'name' ) );
		$type = trim( $node->attributes( 'type' ) );
		$Def = trim( $node->attributes( 'default' ) );
		...
		{ /* @var $element JFilterElement */
			$id = $type . '_' . $name;
			return $element->render( $name, $id, $node, $this->_config );
		}
	...
	public function loadSetupFile( $path )
	{
		...
		if ( $path )
		{
			...
			{
				if ( isset( $xml->document->filters ) && $params = $xml->document->filters )
				{
	...
	public function loadElement( $type, $new = false )
	{
		...
		$elementClass = 'FilterElement' . $type;
		if ( !class_exists( $elementClass ) )
		{
			$file = dirname( __FILE__ ) . DS . 'elements' . DS . 'filter' . DS . FilterInput::getInstance()->clean( str_replace( '_', DS, $type ) . '.php', 'path' );
			if ( is_file( $file ) )
			{
				include_once $file;
			}
	...
	{
		echo '<table class="' . $this->_xml[$group]->attributes( 'class' ) . '">';
	...
	public function getVerticalTable( $group )
	{
		if ( !isset( $this->_xml[$group] ) )
		{
			return false;
		...
		<div class="row filter-block">
			...
			foreach ( $children as $param )
			{
				$class = $param->attributes( 'class' ) ? ' class="' . $param->attributes( 'class' ) . '" ' : '';
				$label = $param->attributes( 'label' ) ? $param->attributes( 'label' ) : '';
				$name = trim( $param->attributes( 'name' ) );
				$must = trim( $param->attributes( 'must' ) );
				$type = trim( $param->attributes( 'type' ) );
				...
				<div class="form-group">
					<label class="control-label col-sm-5" for="<?php echo $id; ?>">
						<?php
						echo Text::_( $label );
						if ( $must == 1 )
						{
