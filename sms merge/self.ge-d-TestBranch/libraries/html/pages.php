...
require_once 'elements' . DS . 'page.php';
...
class Page extends Registry
{
    ...
    public function __construct($data, $path = '', $config = array())
    {
        ...
        // Set base path
        $this->_elementPath[] = dirname(__FILE__) . DS . 'elements' . DS . 'page';
        if(!empty($data))
        {
            $this->_data = $data;
        }
    ...
    public function getParam($node, $row, $group = '_default')
    {
        ...
        {
            echo $element->render($row, $node, $this->_config);
        }
    }
    ...
    public function loadElement($type, $new = false)
    {
        ...
        if(!class_exists($elementClass))
        {
            ...
            $file = dirname(__FILE__) . DS . 'elements' . DS . 'page' . DS . FilterInput::getInstance()->clean(str_replace('_', DS, $type) . '.php', 'path');
