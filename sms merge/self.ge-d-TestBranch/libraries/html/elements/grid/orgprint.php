<?php

/**
 * @version		$Id: list.php 1 2011-07-13 05:09:23Z $
 * @package	WSCMS.Framework
 * @copyright	Copyright (C) 2009 - 2010 WebSolutions. All rights reserved.
 * @license		GNU General Public License version 2 or later
 */
// Check to ensure this file is within the rest of the framework
/**
 * Renders a list element
 *
 * @package 	WSCMS.Framework
 * @subpackage		Parameter
 * @since		1.5
 */
class JGridElementOrgPrint extends JGridElement
{
	/**
	 * Element type
	 *
	 * @access	protected
	 * @var		string
	 */
	protected $_name = 'Print';

	public function fetchElement( $row, $node, $group )
	{
		/* @var $node SimpleXMLElements */
		$key = trim( $node->attributes( 'key' ) );
		$Parts = explode( '.', $key );
		$Base = (array) C::_( C::_( '0', $Parts ), $row, array() );

		$LimitType = trim( $node->attributes( 'limit_type' ) );
		$Length = intval( $node->attributes( 'length' ) );
		$Text = '';
		$HTML = array();
		if ( count( $Base ) )
		{
			foreach ( $Base as $Item )
			{
				$Text = C::_( C::_( '1', $Parts ), $Item );

				switch ( $LimitType )
				{
					default:
					case 1:
						$Ttext = Helper::limitWords( $Text, $Length );
						break;
					case 2:
						$Ttext = Helper::limitLetters( $Text, $Length );
						break;
				}

				if ( empty( $Text ) )
				{
					$SText = '&nbsp;';
				}
				else
				{
					$SText = Helper::MakeDoubleToolTip( $Ttext, $Text );
				}

				$HTML[] = '<div class="key_div">'
								. '<span class="key_val">'
								. $SText
								. '</span>'
								. '</div>'
				;
			}
		}
		return '<div class="key_row">' . implode( '', $HTML ) . '</div>';

	}

}
