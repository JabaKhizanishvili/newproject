<?php
/**
 * @version    $Id: date.php,v 1.1 2013/07/04 06:23:52 temo Exp $
...
class PDate extends DateTime
{
	 ...
	 * @throws	JException
	 */
	public function __construct( $date = 'now', $tz = 4 )
	{
	...
	public function setTimestamp( $UnixTimestamp, $tz = 4 )
	{
		if ( is_numeric( $UnixTimestamp ) )
		{
			...
			return parent::__construct( date( 'c', $UnixTimestamp ), $tz );
		}
		return false;
	 ...
	 * @since	1.5
	 */
	protected function dayToString( $day, $abbr = false, $Translate = true )
	{
		if ( $Translate )
		{
			switch ( $day )
	 ...
	 * @since	1.5
	 */
	protected function monthToString( $month, $abbr = false, $Translate = true )
	{
		if ( $Translate )
		{
	 ...
	 * @since	1.5
	 */
	public function toFormat( $format = '%Y-%m-%d %H:%M:%S', $local = true, $Translate = true )
	{
		...
		{
			$format = str_replace( '%a', $this->dayToString( date( 'w', $time ), true, $Translate ), $format );
		}
		if ( strpos( $format, '%A' ) !== false )
		{
			$format = str_replace( '%A', $this->dayToString( date( 'w', $time ), false, $Translate ), $format );
		}
		if ( strpos( $format, '%b' ) !== false )
		{
			$format = str_replace( '%b', $this->monthToString( date( 'n', $time ), true, $Translate ), $format );
		}
		if ( strpos( $format, '%B' ) !== false )
		{
			$format = str_replace( '%B', $this->monthToString( date( 'n', $time ), false, $Translate ), $format );
		}
	...
	public static function StringDate( $start = null, $end = null, $delimiter = ', ' )
	{
		...
		foreach ( $prepare as $key => $val )
		{
			if ( $diff->$key || $print )
			{
				$prepare[$key] = Text::sprintf( $val, $diff->$key );
				$print = 1;
			}
			else
			{
		...
		$prepare = implode( $delimiter, $prepare );
		$mode = $diff->format( '%R' );
		if ( $prepare && $mode == '+' )
		{
			$prepare .= ' ' . Text::_( 'AFTER' );
		}
		elseif ( $prepare )
		{
			$prepare .= ' ' . Text::_( 'BEFORE' );
		}
		return $prepare;
