 ...
 * for the WSCMS Framework to access and manipulate a URI.  Second to attain the URI of
		...
		if ( !isset( $instances[$uri] ) )
		{
			...
			if ( $uri == 'SERVER' )
			{
				...
				else
				{
					...
					$theURI = 'http' . $https . $HTTP_HOST . $_SERVER['SCRIPT_NAME'];
				...
				$theURI0 = urldecode( $theURI );
				$theURI1 = str_replace( '"', '&quot;', $theURI0 );
				$theURI2 = str_replace( '<', '&lt;', $theURI1 );
				$theURI3 = str_replace( '>', '&gt;', $theURI2 );
				$theURI4 = preg_replace( '/eval\((.*)\)/', '', $theURI3 );
				$theURI = preg_replace( '/[\\\"\\\'][\\s]*javascript:(.*)[\\\"\\\']/', '""', $theURI4 );
			}
			else
			{
				// We were given a URI
