<?php
/**
 * @version		$Id: request.php 1 2011-07-13 05:09:23Z $
...
//require_once dirname( __FILE__ ) . DS . 'helpers' . DS . 'filterinput.php';
//require_once dirname( __FILE__ ) . DS . 'helpers' . DS . 'filteroutput.php';
//require_once dirname( __FILE__ ) . DS . 'arrayhelper.php';
/**
 ...
 * JRequest Class
 *
 * This class serves to provide the WSCMS Framework with a common interface to access
...
class Request
{
	 ...
	 * @since	1.5
	 */
	public static function getVar( $name, $default = null, $hash = 'request', $type = 'none', $mask = 0 )
	{
		...
		$GLOBALS['_REQUEST'][$name] = C::_( '_REQUEST.' . $name, $GLOBALS, array() );
		...
		else if ( !isset( $GLOBALS['_REQUEST'][$name][$sig] ) )
		{
...
//				}
	 ...
	 * @since	1.5
	 */
	public static function getUnsigned( $name, $default = 0, $hash = 'default' )
	{
		return self::getVar( $name, $default, $hash, 'unsigned' );
	 ...
	 * @since	1.5
	 */
	public static function setVar( $name, $value = null, $hash = 'get', $overwrite = true )
	{
		$mask = 0;
		$type = strtoupper( 'none' );
		...
		$GLOBALS['_REQUEST'][$name] = C::_( '_REQUEST.' . $name, $GLOBALS, array() );
		$GLOBALS['_REQUEST'][$name][$Key] = true;
...
//		}
	...
	public static function clean()
	{
		...
		// Make sure the request hash is clean on file  inclusi on
		$GLOBALS['_REQUEST'] = array();
	...
	protected static function _cleanArray( &$array, $globalise = false )
	{
		...
		foreach ( $array as $key => $value )
		{
			...
			$failed = in_array( mb_strtolower( $key ), $banned );
	...
	{
		$old_state = self:: _getState( $request, NULL, $key );
		$cur_state = (!is_null( $old_state )) ? $old_state : $default;
		$new_state = self:: getVar( $request, null, 'default', $type );
		// Save the new value only if it was set in this request       
		if ( $new_state !== null )
		{
			self:: _setState( $request, $new_state, $key );
		}
		else
		{
			$new_state = $cur_state;
	...
	public static function _setState( $var, $value = NULL, $key = 'default' )
	{
		$s = Session::Get( $key );
		$s[$var] = $value;
		Session::Set( $key, $s );
	...
	public static function _getState( $var, $value = NULL, $key = 'default' )
	{
		$s = Session::Get( $key );
		if ( isset( $s[$var] ) )
		{
