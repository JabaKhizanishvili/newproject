...
class Job_descriptionsModel extends Model
{
    function getList()
    {
		...
		if ( $Return->lib_title )
		{
			$where[] = ' t.id in (' . $this->_search( $Return->lib_title, [ 'lib_title' ], 'lib_job_descriptions' ) . ')';
		}
		if ( $Return->active > -1 )
		{
        ...
        $countQuery = 'select count(*) from  lib_job_descriptions t '
                . $whereQ
        ;
