...
class Working_ratesModel extends Model
{
    ...
    function getList()
    {
		...
		if ( $Return->lib_title )
		{
			$where[] = ' t.id in (' . $this->_search( $Return->lib_title, [ 'lib_title' ], 'lib_working_rates' ) . ')';
		}
		if ( $Return->active > -1 )
		{
        ...
        $countQuery = 'select count(*) from  lib_working_rates t '
                . $whereQ
        ;
