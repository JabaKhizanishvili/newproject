...
if ( !$All )
{
	$Query .= ' and (to_date(k.event_date, \'hh24:mi:ss dd-mm-yyyy\') + 1 / 24 * ' . $Hour . ') <= sysdate '
					. ' and (to_date(k.event_date, \'hh24:mi:ss dd-mm-yyyy\') + 1 / 24 * ' . ($Hour + 3) . ') >= sysdate ';
}
...
if ( !empty( $items ) )
{
	...
	foreach ( $items as $Worker )
	{
		...
		$BLTable = clone $Table;
		$BID = C::_( 'BID', $Worker, null );
		$IS_NEW = C::_( 'IS_NEW', $Worker, null );
		$ExpiryDate = new PDate( $EVENT_DATE . ' + 7 day' );
		...
		if ( $IS_NEW )
		{
			...
			{
				$BLTable->load( $BID );
				$BLTable->EXPIRY_DATE = $ExpiryDate->toFormat( '%Y-%m-%d 23:59:59' );
			}
			else
			{
				$BLTable->WORKER = C::_( 'ID', $Worker );
				$BLTable->EXPIRY_DATE = $ExpiryDate->toFormat( '%Y-%m-%d 23:59:59' );
				$BLTable->LISTED_DATE = $EVENT_DATE->toFormat();
			}
